        ...
        public async Task MonitorDistributedRequests(CancellationToken cancelationToken)
        {
            ...
            while(!cancelationToken.IsCancellationRequested)
            {
                ...
                foreach(var request in executingRequests)
                {
                    ...
                    var results = childRequests.Where(x => x.PipelineStepId == reloadedRequest.EndingStepId).Join(_context.OperationResponses, x => x.Id, x => x.Id, (x, y) => JsonConvert.DeserializeObject(y.Result, outputType));
